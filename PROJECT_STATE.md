...
- The Rust Offdesk CLI decomposition now keeps clap value parsing in
  ...
  Registration, review, and launch-preparation command coordination now lives
...
0. Continue splitting the large Offdesk CLI (`src/cli/offdesk.rs`, ~9.1k
   ...
   main handler retains output and file-write side effects. Offdesk deck source
   classification, artifact path allocation, Markdown presentation, Marp
   ...
   never executes a mutation. Adaptive-wiki entry/candidate list and detail
   argument parsing, human-projection query coordination, exact-id selection,
   and JSON/terminal presentation now live in the bounded read-only
   `src/cli/offdesk/wiki_catalog.rs` adapter. Catalog reads preserve canonical
   entry and candidate bytes, and a missing detail id fails closed.
   Adaptive-wiki mutation result envelopes and JSON/terminal presentation now
   live in `src/cli/offdesk/wiki_mutation_presentation.rs`; validation,
   canonical writes, audit appends, and promotion receipt creation remain in
   the command handlers. JSON mutation contracts remain covered, and the
   human retag path now has an integration regression. Adaptive-wiki runtime
   policy acknowledgement report read models, suggested-action command
   templates, and JSON/terminal presentation now live in
   `src/cli/offdesk/wiki_runtime_policy_ack_presentation.rs`; store reads,
   ...
   projection store reads remain in the report handler, and review renewal
   ...
   creation. Adaptive-wiki episode evaluation, live trace,
   correction-recurrence, and promotion-chain terminal/JSON presentation now
   live in `src/cli/offdesk/wiki_evidence_presentation.rs`; query and filter
   construction, profile and store selection, report generation, and artifact
