...
- `sync_policy.sample.json` is copied as `.aoe-team/sync_policy.sample.json`. Rename to `.aoe-team/sync_policy.json` only when a project needs path/confidence overrides for `/sync`. Supported knobs include `exclude_globs`, `include_globs`, `class_confidence`, `doc_type_confidence`, `group_overrides`, and `min_confidence`.
